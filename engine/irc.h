...
struct Server {
	...
	bool connect(const char *host, const char *nick, const char *passwd, int port = 6667, const char *alias = NULL);
...
struct Client {
	...
	bool connect(const char *host, const char *nick, const char *pass, int port = 6667, const char *alias = NULL) {
		Server *s = new Server;
		s->client = this;
		s->init();
		s->connect(host, nick, pass, port, alias);
		servers.push_back(s);
		return true;
	}
...
static void irc_eventcb(struct bufferevent *buf, short what, void *arg) {
	...
	if(what == BEV_EVENT_CONNECTED) {
		...
		if(s->pass && s->pass[0]) DEBUGF(evbuffer_add_printf(eb, "PASS %s\r\n", s->pass));
...
bool Server::connect(const char *h, const char *n, const char *w, int p, const char *alias_) {
	host = strdup(h);
	port = p;
	pass = w&&w[0]?strdup(w):0;
