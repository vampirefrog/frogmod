...
bool skipirc = false;
int lastupdateirc = 0;
struct frog_irc_channel_t {
   string channel, key;
};
struct frog_irc_session_t {
   bool ready, disconnected;
   string alias, host;
   int port;
   string pass, nick, username, realname;
   string connectcmd;
   irc_session_t *session;
   vector <frog_irc_channel_t> join_queue;
};
vector <frog_irc_session_t> frogsessions;
void irc_event_connect(irc_session_t * session, const char * event, const char * origin, const char ** params, unsigned int count) {
   loopv(frogsessions) {
       if(frogsessions[i].session == session) {
           frogsessions[i].ready = true;
           frogsessions[i].disconnected = false;
           if(frogsessions[i].connectcmd[0]) {
               irc_send_raw(session, "%s", frogsessions[i].connectcmd);
           }
           loopvj(frogsessions[i].join_queue) {
               irc_cmd_join(session, frogsessions[i].join_queue[j].channel, 0);
           }
           break;
       }
   }
...
void irc_event_channel(irc_session_t * session, const char * event, const char * origin, const char ** params, unsigned int count) {
   skipirc = true;
   server::sendservmsgf("<%s> %s", origin, params[1]);
   skipirc = false;
}

static void frog_irc_session_connect(frog_irc_session_t *frogsession) {
    if(frogsession->session)
        irc_destroy_session(frogsession->session);

    irc_callbacks_t irccallbacks;
    memset(&irccallbacks, 0, sizeof(irccallbacks));
    irccallbacks.event_connect = irc_event_connect;
    irccallbacks.event_channel = irc_event_channel;

    frogsession->session = irc_create_session(&irccallbacks);
    if(!frogsession->session) {
        conoutf("Could not create IRC session!");
        return;
    }
    irc_option_set(frogsession->session, LIBIRC_OPTION_STRIPNICKS | LIBIRC_OPTION_DEBUG | LIBIRC_OPTION_SSL_NO_VERIFY);
    if(irc_connect(frogsession->session, frogsession->host, frogsession->port, frogsession->pass, frogsession->nick, frogsession->username, frogsession->realname)) {
        conoutf("Could not connect to IRC: %s (%d)", irc_strerror(irc_errno(frogsession->session)), irc_errno(frogsession->session));
    }
}

static void updateirc() {
    loopv(frogsessions) {
        if(frogsessions[i].disconnected) {
            conoutf("Reconnecting to IRC %s %s:%d", frogsessions[i].alias, frogsessions[i].host, frogsessions[i].port);
            frog_irc_session_connect(&frogsessions[i]);
        }
    }
    lastupdateirc = 0;
}

ICOMMAND(ircconnect, "ssisssss", (const char *alias, const char *host, int *port, const char *pass, const char *nick, const char *username, const char *realname, const char *connectcmd), {
    if(alias && *alias && host && *host) {
        conoutf("Connecting to IRC %s %s\n", alias, host);
        frog_irc_session_t frogsess;
        frogsess.session = NULL;
        frogsess.ready = false;
        frogsess.disconnected = false;
        copystring(frogsess.alias, alias);
        copystring(frogsess.host, host);
        frogsess.port = port && *port ? *port : 6667;
        copystring(frogsess.pass, pass&&*pass?pass:"");
        copystring(frogsess.nick, nick&&*nick?nick:"frogbot");
        copystring(frogsess.username, username&&*username?username:"frogbot");
        copystring(frogsess.realname, realname&&*username?username:"frogbot");
        copystring(frogsess.connectcmd, connectcmd&&*connectcmd?connectcmd:"");
        frog_irc_session_connect(&frogsess);
        frogsessions.add(frogsess);
    }
});
ICOMMAND(ircjoin, "sss", (const char *alias, const char *channel, const char *key), {
   if(alias && *alias && channel && *channel) {
       frog_irc_session_t *s = 0;
       loopv(frogsessions) {
           if(!strcmp(frogsessions[i].alias, alias)) {
               s = &frogsessions[i];
               break;
           }
       }
       if(s) {
           if(s->ready)
               irc_cmd_join(s->session, channel, key&&*key?key:0);
           else {
               frog_irc_channel_t chan;
               copystring(chan.channel, channel);
               copystring(chan.key, key);
               s->join_queue.add(chan);
           }
       }
   }
});
ICOMMAND(ircpart, "ss", (const char *alias, const char *channel), {
   if(alias && *alias && channel && *channel) {
       frog_irc_session_t *s = 0;
       loopv(frogsessions) {
           if(!strcmp(frogsessions[i].alias, alias)) s = &frogsessions[i];
       }
       if(s) {
           irc_cmd_part(s->session, channel);
       }
   }
});
void frog_irc_out(const char *str) {
    string str8;
    size_t e = encodeutf8((uchar *)str8, sizeof(str8), (uchar *)str, strlen(str));
    str8[e] = 0;
    loopv(frogsessions) {
        loopvj(frogsessions[i].join_queue) {
            irc_cmd_msg(frogsessions[i].session, frogsessions[i].join_queue[j].channel, str8);
        }
    }
}
void frog_irc_outv(const char *fmt, va_list args) {
   string str;
   vformatstring(str, fmt, args, sizeof(str));
   frog_irc_out(str);
}
...
void rconcb(httprequest *req, void *data) {
//    string ipstr;
...
//    conoutf("%s %s %s (size=%d)", ipstr, req->method.data, req->path.data, rconreqs.length());
    if(!strcmp((char *)req->method, "POST")) {
       ...
       rconreqs.add(req);
       // do not end, leave pending
    }
}

void rcon_conn_lost_cb(httprequest *req) {
    loopv(rconreqs) {
        if(rconreqs[i] == req) rconreqs.remove(i--);
    }
}
...
void checkserversockets()        // reply all server info requests
{
    ...
    if(rconport && http) {
        maxsock = max(maxsock, http->add_to_sockset(readset));
    }
    loopv(frogsessions) {
        if(irc_is_connected(frogsessions[i].session)) {
            irc_add_select_descriptors(frogsessions[i].session, &readset, &writeset, &maxsock);
        } else {
//            conoutf("irc not connected!");
        }
    }

    if(enet_socketset_select(maxsock, &readset, &writeset, 0) <= 0) return;
    if(rconport && http) {
        http->check_sockset(readset);
    }
    ...
    if(mastersock != ENET_SOCKET_NULL)
    {
        ...
        loopv(frogsessions) {
            if(irc_process_select_descriptors(frogsessions[i].session, &readset, &writeset)) {
                if(!frogsessions[i].disconnected && !lastupdateirc) {
                    frogsessions[i].disconnected = true;
                    lastupdateirc = totalmillis;
...
void serverslice(bool dedicated, uint timeout)   // main server update, called from main loop in sp, or from below in dedicated server
{
    ...
    if(!lastupdatemaster || totalmillis-lastupdatemaster>60*60*1000) {
        // send alive signal to masterserver every hour of uptime
        updatemasterserver();
    }

    if(lastupdateirc && totalmillis - lastupdateirc > 5 * 1000) {
        updateirc();
    }
